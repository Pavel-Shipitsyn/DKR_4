...
begin
  var fo: real;
  fo := 2 * power(x,3) + (-2) * power(x,2) + 3 * x + 8;
  fun := fo;
end;
...
begin
  var fo: real;
  fo := (2*x * (power(x,4)/4) + ((-2)*x) * (power(x,3)/3) + (3*x*x)/2 + 8 * x);
  fun1 := fo;
...
begin
  ...
  var ss: string;
  clearwindow;
  textout(10, 10, 'Вычисление площади фигуры, ограниченной кривой 2* x^3 + (-2) * x^2 + 3*x+8 и осью Ох (в положительной части по оси Оу)');
...
begin        
  MaximizeWindow;
  Writeln('Вычисление площади фигуры, ограниченной кривой 2* x^3 + (-2) * x^2 + 3*x+8 и осью Ох (в положительной части по оси Оу)');
